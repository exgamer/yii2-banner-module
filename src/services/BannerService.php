...
class BannerService extends Service
{
    ...
    public function getBannersForCurrentUrl()
    {
        ...
        $modelClass::$search_by_locale_callable = function($q, $localizedAlias) {
            $q->orWhere("{$localizedAlias}.from_at is null and {$localizedAlias}.to_at is null");
            $q->orWhere("now() between {$localizedAlias}.from_at and {$localizedAlias}.to_at and {$localizedAlias}.from_at is not null and {$localizedAlias}.to_at is not null");
            $q->orWhere("{$localizedAlias}.from_at < now() and {$localizedAlias}.from_at is not null and {$localizedAlias}.to_at is  null");
            $q->orWhere("{$localizedAlias}.to_at > now() and {$localizedAlias}.from_at is  null and {$localizedAlias}.to_at is not null");
        };
